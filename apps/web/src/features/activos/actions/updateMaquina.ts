...
import { maquinaSchema } from "../schemas/maquinaSchema";
import {
...
const maquinaUpdateSchema = maquinaSchema.and(
  z.object({ id: z.string().uuid("Activo inválido.") })
);

export type MaquinaUpdateInput = z.infer<typeof maquinaUpdateSchema>;

export async function updateMaquina(
  input: MaquinaUpdateInput
): Promise<ResultadoActivo> {
  await requireUser();
  if (!(await autorizar("activos.maquinas.editar"))) {
    return { ok: false, error: "No tienes permiso para editar maquinaria." };
  }

  const parsed = maquinaUpdateSchema.safeParse(input);
  if (!parsed.success) {
