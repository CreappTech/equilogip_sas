...
import { maquinaSchema, type MaquinaInput } from "../schemas/maquinaSchema";
import {
  autorizar,
...
export async function createMaquina(
  ...
  await requireUser();
  if (!(await autorizar("activos.maquinas.crear"))) {
    return { ok: false, error: "No tienes permiso para crear maquinaria." };
  }
